"""
core/buy.py — btb_cli 独立抢票核心模块

与主仓库 task/buy.py 对齐的关键修复 / 特性：
  [BUG FIX #963] 429 限流：收到 BiliRateLimitError 后按 rate_limit_delay_ms 冷却再继续，
    避免「光速重试」瞬间触发 412 终极 IP 风控。
  [BUG FIX #977] 推送丢失：抢票成功 start_all() 后追加 join_all(timeout=...)，
    阻塞主进程等待推送线程真正发完，避免 CLI 退出时 daemon 线程被强杀。
  [BUG FIX #5a32af7] 412 in order/prepare：准备订单阶段 .json() 抛 JSONDecodeError 时，
    走 handle_non_json_response 识别 412 并切换代理 / 进入冷却，而非直接当作程序异常。
  [FEAT] 代理 API 自动补池（ProxyApiProvider）、循环内随机次数主动复检项目详情。
"""
...
class Buy:
    ...
    def stream(self):
        yield from buy_stream(self.config.resolved_config())
    ...
    while isRunning:
        try:
            ...
            while attempt <= effective_retry_limit:
                ...
                while attempt <= batch_end:
                    ...
                    try:
                        ...
                        # [BUG FIX #963] 收到 429 后必须冷却再继续，避免光速重试触发 412 终极风控。
                        # 与主仓库对齐：按 rate_limit_delay_ms（默认 100ms）冷却后 continue。
                        # 如需更保守，可将 --rate-limit-delay-ms 调大（例如等于 --interval）。
            ...
            if errno == 0:
                ...
                # [BUG FIX #977] 让 Server酱/Bark/PushPlus 等渠道的 HTTP 请求有时间发完，
                # 否则 CLI 退出时 daemon 推送线程会被强杀。先弹码再 join，不阻塞用户看二维码。
            ...
            # [BUG FIX #5a32af7] 准备订单接口返回非 JSON（典型为 412 风控的 Brotli/HTML 页面）。
            # 走 handle_non_json_response 识别 412 并切换代理 / 进入冷却，避免直接当作程序异常。
