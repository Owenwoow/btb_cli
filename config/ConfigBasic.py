...
class BasicConfig:
    """通用配置基类，支持 from_env()、from_mapping()、with_overrides()、to_cli_args()"""
    ...
    @classmethod
    def from_mapping(cls, source: dict[str, Any], *, source_name: str):
        kwargs: dict[str, Any] = {}
        for f in fields(cls):
            if not f.init:
                continue
            if cls._is_nested_config_field(f):
                ...
                kwargs[f.name] = nested_cls.from_mapping(source, source_name=source_name)
                continue
            ...
            kwargs[f.name] = cls._normalize_value(f, raw, source_name=source_name)
        return cls(**kwargs)
    ...
    def from_env(cls):
        ...
        for f in fields(cls):
            ...
            kwargs[f.name] = cls._normalize_value(f, raw, source_name="env")
        return cls(**kwargs)
